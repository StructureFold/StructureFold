...
def fold_RNA(mode, p_type, id_s, temperature, slope, intercept, path):
    ...
    if mode == '1':
        ...
        if not p_type:
            ...
            suffix = "restraint"
        os.system(os.path.join(vp, "RNAfold")+" < "+os.path.join(path,id_s+"_temp.fa")+" -T "+str(float(temperature)-273.15)+" > "+os.path.join(dbn_path, t+"_"+temperature+"_"+suffix+".dbn"))
...
def __main__():
    ...
    if not predict_type:
        ...
        output_name = os.path.basename(predict_type)+"_"+os.path.basename(id_file)+"_"+temperature+"_"+os.path.basename(seq_file)+"_"+program[mode]+mf.strip()+"_sht_"+str(shift)
